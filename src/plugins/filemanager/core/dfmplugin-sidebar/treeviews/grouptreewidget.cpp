...
QSharedPointer<SideBarModel> GroupTreeWidget::kSidebarModelIns { nullptr };
...
void GroupTreeWidget::setCurrentUrl(const QUrl &url)
{
    sidebarView->setCurrentUrl(url);
}

QUrl GroupTreeWidget::currentUrl() const
{
    return sidebarView->currentUrl();
}

QAbstractItemView *GroupTreeWidget::view()
{
    return sidebarView;
}

int GroupTreeWidget::addItem(SideBarItem *item)
{
...
bool GroupTreeWidget::insertItem(const int index, SideBarItem *item)
{
    Q_ASSERT(qApp->thread() == QThread::currentThread());
...
bool GroupTreeWidget::removeItem(const QUrl &url)
{
    Q_ASSERT(qApp->thread() == QThread::currentThread());
    return kSidebarModelIns->removeRow(url);
}

void GroupTreeWidget::updateItem(const QUrl &url, const ItemInfo &newInfo)
{
    Q_ASSERT(qApp->thread() == QThread::currentThread());
 ...
 */
int GroupTreeWidget::findItem(const QUrl &url) const
{
...
QModelIndex GroupTreeWidget::findItemIndex(const QUrl &url) const
{
    return sidebarView->findItemIndex(url);
}

void GroupTreeWidget::editItem(const QUrl &url)
{
...
void GroupTreeWidget::setItemVisiable(const QUrl &url, bool visible)
{
...
void GroupTreeWidget::updateItemVisiable(const QVariantMap &states)
{
...
QList<QUrl> GroupTreeWidget::findItems(const QString &group) const
{
    QList<QUrl> ret;
...
void GroupTreeWidget::updateSelection()
{
...
void GroupTreeWidget::saveStateWhenClose()
{
    sidebarView->saveStateWhenClose();
}

void GroupTreeWidget::onItemActived(const QModelIndex &index)
{
...
void GroupTreeWidget::customContextMenuCall(const QPoint &pos)
{
...
void GroupTreeWidget::onItemRenamed(const QModelIndex &index, const QString &newName)
{
    SideBarItem *item = kSidebarModelIns->itemFromIndex(index);
    if (!item)
        return;
...
void GroupTreeWidget::initializeUi()
{
...
void GroupTreeWidget::initDefaultModel()
{
...
void GroupTreeWidget::initConnect()
{
    // do `cd` work
    connect(sidebarView, &SideBarView::activated,
            this, &GroupTreeWidget::onItemActived);
    ...
    connect(sidebarView, &SideBarView::customContextMenuRequested,
            this, &GroupTreeWidget::customContextMenuCall);

    auto delegate = qobject_cast<SideBarItemDelegate *>(sidebarView->itemDelegate());
    if (delegate) {
        connect(delegate, &SideBarItemDelegate::rename, this, &GroupTreeWidget::onItemRenamed);
